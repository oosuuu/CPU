...
//Jacobi
    for (n = 0; n < STEPS; n++){
        ...
        //non-block correspondence
        MPI_Isend(&b[end_row][1], TS-2, MPI_DOUBLE, down, tag1, MPI_COMM_WORLD, &req[0]);
        MPI_Isend(&b[begin_row][1], TS-2, MPI_DOUBLE, up, tag2, MPI_COMM_WORLD, &req[1]);
        MPI_Irecv(&a[0][1], TS-2, MPI_DOUBLE, up, tag1, MPI_COMM_WORLD, &req[2]);
        MPI_Irecv(&a[MS+1][1], TS-2, MPI_DOUBLE, down, tag2, MPI_COMM_WORLD, &req[3]);
        ...
        //complete non_block correspondence
        for (i = 0; i < 4; i++){
